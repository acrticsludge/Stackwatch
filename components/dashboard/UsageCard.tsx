import { getProgressColorClass, getStatusColorClass, relativeTime, METRIC_LABELS, METRIC_UNITS, SERVICE_LABELS } from "@/lib/utils";
...
export function UsageCard({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      {/* Progress bar */}
      <div className="h-2 w-full rounded-full bg-slate-100 mb-2 overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${getProgressColorClass(pct)}`}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs font-semibold ${getStatusColorClass(pct)}`}>
          {pct}% used
        </span>
      </div>
