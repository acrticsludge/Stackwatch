...
function MiniUsageCard({ icon, service, label, metric, current, limit, unit, pct }: MiniCardProps) {
  const barColor = pct >= 80 ? "bg-red-500" : pct >= 60 ? "bg-amber-500" : "bg-blue-500";
  const pctColor = pct >= 80 ? "text-red-400" : pct >= 60 ? "text-amber-400" : "text-green-400";
  const dotColor = pct >= 80 ? "bg-red-500" : pct >= 60 ? "bg-amber-500" : "bg-green-500";
    ...
    <div className="bg-[#111] border border-white/[0.07] rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs font-medium text-zinc-200 leading-tight">{service}</p>
            <p className="text-[10px] text-zinc-600">{label}</p>
          </div>
        </div>
        <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${dotColor} ${pct >= 80 ? "animate-pulse" : ""}`} />
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-[11px] font-semibold tabular-nums ${pctColor}`}>{pct}%</span>
      </div>
    </div>
  );
}
          ...
          <div className="relative">
            ...
            <div className="absolute -top-5 -left-3 z-10 max-w-[255px] bg-[#161616] border border-amber-500/20 rounded-xl px-3.5 py-2.5 shadow-2xl shadow-black/50 flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-zinc-200">GitHub Actions · 94%</p>
            ...
            <div className="mt-2.5 bg-[#111] border border-white/[0.06] rounded-xl px-4 py-2.5 flex items-center justify-between">
              ...
              <span className="text-[10px] text-zinc-700 font-mono">synced 1m ago</span>
            </div>
          </div>
