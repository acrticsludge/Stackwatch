...
import { METRIC_LABELS, METRIC_UNITS, SERVICE_LABELS, relativeTime } from "@/lib/utils";
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-medium text-zinc-100">{SERVICE_LABELS[service] ?? service}</p>
                ...
                return (
                  <div key={snap.metric_name}>
                    ...
                    <span className="text-xs text-zinc-600">
                      {snap.current_value.toLocaleString()} / {snap.limit_value.toLocaleString()} {unit}
                    </span>
                  </div>
                );
              })}
      ...
      <div className="pt-3 border-t border-white/4 flex items-center justify-between">
        ...
        <span
          className={`inline-flex items-center gap-1.5 text-xs ${
            status === "connected" ? "text-green-500" : status === "error" ? "text-red-400" : "text-zinc-600"
          }`}
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              status === "connected" ? "bg-green-500" : status === "error" ? "bg-red-500" : "bg-zinc-600"
            }`}
          />
          {status}
        </span>
